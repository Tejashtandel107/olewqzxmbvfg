    ...
    <div class="card">
        ...
        <div id="notify"></div>
            <div class="row mb-2 justify-content-between">
               <div class="col-auto">
                    {{ Form::model($request,array('url' =>route('account-manager.clients.index'),'method'=>'get','id'=>'form-filter')) }} 
                ...
                <div class="col-auto align-self-end">
                    <div class="text-xl-end mt-3">
                        <a href="{{ route('account-manager.clients.create') }}"><x-primary-button><i class="mdi mdi-plus me-1"></i>{{__('Add New')}}</x-primary-button></a>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-centered table-striped text-nowrap">
                        ...
                        @if(isset($clients) && $clients->count()>0)
                            @foreach($clients as $client)
                                <tr>
                                    ...
                                    <td>{{ $client->email }}</td>
                                    <td><x-badge :status="$client->status"/></td>
                                    <td>{{ $client->created_at }}</td>
                                    <td>
                                        <a href="{{ route('account-manager.clients.edit',$client->user_id) }}" class="action-icon"> <i class="mdi mdi-square-edit-outline"></i></a>
                                        ...
                                        <a href="{{ route('account-manager.companies.index',['client_id'=>$client->user_id]) }}" class="action-icon"> <i class="mdi mdi-office-building-outline"></i></a>                                
                                    </td>
                                </tr>
                            @endforeach
                        @else
...
@section('page-scripts')
    <script src="{{ asset('/assets/js/account-manager/client/index.min.js') }}"></script>
@endsection
</x-app-layout>
