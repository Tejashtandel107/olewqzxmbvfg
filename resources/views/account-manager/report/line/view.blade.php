...
@endsection
    <div class="card">
        <div class="card-body">
            <div>
                {{ Form::model($request,array('url' =>route("account-manager.reports.lines"),'method'=>'get','id'=>'form-filter')) }} 
                    ...
                    <div class="form-control-width">
                        ...
                        <select name="operator_id" class="select2 form-control" data-toggle="select2">
                            ...
                            <optgroup label='{{trans_choice("Account Manager|Account Manager",1)}}'>
                                <option {{($accountManager->user_id==$selected_operator_id) ? 'selected':''}} value="{{$accountManager->user_id}}">{{$accountManager->name}}</option>
                            </optgroup>
