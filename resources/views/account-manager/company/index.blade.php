    ...
    <div class="card">
        ...
        <div id="notify"></div>
            <div class="row mb-2 justify-content-between">
                <div class="col-auto">
                {{ Form::model($request,array('url' =>route('account-manager.companies.index'),'method'=>'get','id'=>'form-filter')) }} 
                ...
                <div class="col-auto align-self-end">
                    <div class="text-xl-end mt-3">
                        <a href="{{ route('account-manager.companies.create') }}" class="btn btn-primary"><i class="mdi mdi-plus me-1"></i>{{__('Add New')}}</a>
                    </div>
                </div>
            </div>
            <div class="row">
                        ...
                        @if(isset($companies) && $companies->count()>0)
                            @foreach($companies as $comapny)
                                <tr>
                                    ...
                                    <td>{{ ($comapny->name) ?? "n/a" }}</td>
                                    <td>{{ $comapny->company_type }}</td>
                                    <td>{{ $comapny->vat_tax}}</td>
                                    <td>{{ $comapny->created_at }}</td>
                                    <td>
                                        <a href="{{ route('account-manager.companies.edit',$comapny->company_id) }}" class="action-icon"> <i class="mdi mdi-square-edit-outline"></i></a>
...
@section('page-scripts')
    <script src="{{ asset('assets/js/account-manager/company/index.min.js') }}"></script>
@endsection
</x-app-layout>
