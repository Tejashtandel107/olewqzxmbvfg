...
class SendLineIncomeInvoice 
{
    ...
    public function handle()
    {
        if($this->lineIncomeMonthly->isInvoiceReady()) {
            if(!empty($this->lineIncomeMonthly->invoice_number)){
                try{
                    ...
                    $this->lineIncomeMonthly->user->notify(new LineIncomeInvoice($this->lineIncomeMonthly,$attachments));
                    $status =  "invoice sent";
                    $notes  = "Fattura inviata con successo allo studio.";
        ...
        if(isset($status)){
            ...
            return ($status=='invoice sent') ? true : false;
        }
        return false;
    }
}
